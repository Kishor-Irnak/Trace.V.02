...
const Pipeline: React.FC = () => {
  ...
  useEffect(() => {
    // Subscribe to real-time updates (mocked)
            ...
            >
              ...
              <div className="mb-4 px-1 flex items-center justify-between group">
                <div className="flex items-center gap-2">
                   <h3 className="text-xs font-semibold text-slate-700 uppercase tracking-wide">{stage.title}</h3>
                   <span className="text-[10px] text-slate-400 bg-slate-100 px-1.5 py-0.5 rounded font-medium">{stageLeads.length}</span>
                </div>
                <div className="flex gap-1 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
                   <button className="p-1 hover:bg-slate-200 rounded text-slate-500 transition-colors"><Plus size={14} /></button>
                </div>
              </div>
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 text-xs text-slate-500">
                          ...
                          <span className="truncate text-slate-400 text-[11px]">{lead.email}</span>
                      </div>
                      ...
                      <div className="flex items-center justify-between pt-2 border-t border-slate-50">
                         ...
                         <div className="flex gap-1">
                            {lead.tags?.slice(0, 2).map(tag => (
                              <Badge key={tag} color="bg-slate-50 border-slate-100 text-slate-500">{tag}</Badge>
                            ))}
                         </div>
                      </div>
                    </GlassCard>
